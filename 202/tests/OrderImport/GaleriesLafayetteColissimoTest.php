...
class GaleriesLafayetteColissimoTest extends AbstractOrdeTestCase
{
    public function testAssociationWithModuleColissimo()
    {
        $apiOrder = $this->getOrderRessourceFromDataset('order-galerieslafayette-colissimo.json');
        ...
        $this->assertEquals($pickupPoint->company_name, 'CARREFOUR CITY');
        $this->assertEquals($pickupPoint->product_code, 'A2P');
        $this->assertEquals($pickupPoint->city, 'TOULOUSE');
    }
}
