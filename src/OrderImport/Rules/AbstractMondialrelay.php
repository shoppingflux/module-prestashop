    ...
    protected function addOrderV3($relayId, $countryIso, $relayData, $carrier, $order, $logPrefix)
    {
        ...
        $insertResult = Db::getInstance()->insert(
            ...
            [
                ...
                'id_cart' => (int) $order->id_cart,
                'id_order' => (int) $order->id,
                'selected_relay_num' => pSQL(trim($formattedRelayId)),
                'selected_relay_adr1' => pSQL(trim($relayData->LgAdr1)),
                'selected_relay_adr2' => pSQL(trim($relayData->LgAdr2)),
                'selected_relay_adr3' => pSQL(trim($relayData->LgAdr3)),
                'selected_relay_adr4' => pSQL(trim($relayData->LgAdr4)),
                'selected_relay_postcode' => pSQL(trim($relayData->CP)),
                'selected_relay_city' => pSQL(trim($relayData->Ville)),
                'selected_relay_country_iso' => pSQL(trim($countryIso)),
